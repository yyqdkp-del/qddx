...
import type { DocListItem } from "@/lib/data/qingdao-mock"

/** 通用文档列表 + 分类筛选（政策法规 / 慈善公益 / 道教常识 / 经典导读复用同款 UI） */
type DocListClientProps = {
...
export function DocListClient({ categories, items, segment }: DocListClientProps) {
  const [activeCategory, setActiveCategory] = useState(categories[0] ?? "全部")

  const filtered =
    activeCategory === "全部" ? items : items.filter((n) => n.category === activeCategory)
